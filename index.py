...
def main_loop():
	for i in range(times):
		...
		try:
			...
			print(url + " exists!")
			f.write("\n" + url + " | STATUS_CODE: " + str(result.getcode()))
...
mode = sys.argv[sys.argv.index('-m') + 1] if '-m' in sys.argv else 'http'
if mode not in ['http', 'https']:
	print("-m argument expected either http or https!")
	sys.exit()
log = '-l' in sys.argv

print("I am going to look for images through " + str(times) + " random URLs with the following domains: " + str(domains))
